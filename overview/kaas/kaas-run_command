...
## **Description:**  
The `kaas-cli run` command executes Kontrol proofs either locally, within a Docker container, or remotely on KaaS infrastructure. Kontrol is a tool for verifying and proving smart contracts and related code artifacts, maintained by Runtime Verification ([Kontrol GitHub repository](https://github.com/runtimeverification/kontrol)).
...
- `--mode, -m`: Specifies the execution mode. Accepted values are `local`, `remote`, or `container`.
    ...
    - **local**: Runs Kontrol proofs directly on the host machine. Requires Kontrol to be installed locally.
    - **remote**: Submits proofs to KaaS remote compute infrastructure, requiring authentication and a valid vault specification.
    - **container**: Executes proofs inside a Docker container, ensuring a consistent and isolated environment ([Docker Docs](https://docs.docker.com/get-started/)).
...
1. **Local Mode:**  
    Calls `kontrol build` and `kontrol prove` directly on the host. Kontrol must be installed beforehand. If Kontrol is not found, the CLI will provide installation guidance ([Kontrol Documentation](https://github.com/runtimeverification/kontrol)).
    
2. **Container Mode:**  
    Uses a Docker container to run `kontrol build` and `kontrol prove`. This ensures a reproducible environment without modifying the host system.
    
3. **Remote Mode:**  
    Uses KaasClient to submit proofs to remote KaaS infrastructure. Requires `--vault-spec` and `--token`. If `--watch` is set, the CLI monitors the job until completion.
...
```bash
kaas-cli run --mode local --extra-build-args "--enable-llvm-debug" --extra-prove-args "--xml-test-report"
```
